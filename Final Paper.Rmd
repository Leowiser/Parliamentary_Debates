...
Therefore, debates about the EU were clearly an emotional topic that lead to heated debates. Iyengar et.al (2012) show that strong emotions not only shape politics but also the society itself and can lead to an increased divide between political partisans.

Interestingly, Rhealut et al. (2016) find in their paper, that the sentiments in parliamentary debates became more positive in the last two hundred years
...
## Data
To answer the question we need the data of the debates in the British parliament. To gather these parliamentary debates, we web scraped the debates in the house of commons and Westminster hall from the TheyWorkForYou website. TheyWorkForYou is run by mySociety a British non for profit organization and aims to make the work of the Uk parliament more accessible and transparent to the wider public (source). In this capacity they also publish a mirrored version of Hansard the official webpage for transcripts of UK debates (source). 
...
```
We validate the performance of the lemmatizer and POS tagger by taking a random sample of speeches, conducting lemmatization and POS tagging manually on each token by ourselves and comparing them with those obtained by the pre-trained models of UDPipe and SpaCy.

[Discuss validation results here]
```{r, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
