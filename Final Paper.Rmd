...
# Upload the web scraped data
df_Debates = read.csv("C:/Users/leonw/OneDrive - KU Leuven/2nd Semester/Collecting Big Data for Social Science/Final Assignment/Parliamentary_Debates_Combined.csv", row.names = 1)

# Delete all duplicated rows
df_Debates_clean <- distinct(df_Debates)
...
df_Debates_clean <- df_Debates_clean[!is.na(df_Debates_clean$speaker_name),]
...
df_value_counts <- df_Debates_clean %>%
...
df_Debates_clean <- df_Debates_clean %>% filter(!speech_content %in% filter_list)

# clean the party column
# Check which parties exist
table(df_Debates_clean$speaker_party)
...
for (search_string in Parties) {
  ...
  df_Debates_clean$Party <- ifelse(grepl(pattern, df_Debates_clean$speaker_party, ignore.case = TRUE), 
                              ifelse(is.na(df_Debates_clean$Party), search_string, 
                                     paste(df_Debates_clean$Party, search_string, sep = ", ")),
                              df_Debates_clean$Party)
}

table(df_Debates_clean$Party)

df_Debates_clean$Party <- gsub("Co-operative, Shadow Minister, Minister", "Co-operative", df_Debates_clean$Party)
df_Debates_clean$Party <- gsub("Green, Shadow Minister, Minister", "Green", df_Debates_clean$Party)
df_Debates_clean$Party <- gsub("Conservative, Minister", "Conservative", df_Debates_clean$Party)
df_Debates_clean$Party <- gsub("Labour, Secretary", "Labour", df_Debates_clean$Party)
df_Debates_clean$Party <- gsub("Labour, Secretary, Shadow Minister, Minister", "Labour", df_Debates_clean$Party)
df_Debates_clean$Party <- gsub("Labour/Co-operative, Co-operative, Labour", "Labour/Co-operative", df_Debates_clean$Party)
df_Debates_clean$Party <- gsub("Liberal Democrat, Liberal Democrats", "Liberal Democrat", df_Debates_clean$Party)
df_Debates_clean$Party <- gsub("Labour, Shadow Minister, Minister", "Labour", df_Debates_clean$Party)
df_Debates_clean$Party <- gsub("Independent, Speaker", "Independent", df_Debates_clean$Party)


table(df_Debates_clean$speaker_name[df_Debates_clean$Party=="Secretary, Shadow Minister, Minister"])
table(df_Debates_clean$speaker_name[df_Debates_clean$Party=="Shadow Minister, Minister"])
table(df_Debates_clean$speaker_name[df_Debates_clean$Party=="Secretary, Shadow Minister, Minister"])
table(df_Debates_clean$speaker_name[df_Debates_clean$Party=="Secretary, Minister"])
table(df_Debates_clean$speaker_name[df_Debates_clean$Party=="Speaker, Secretary"])
table(df_Debates_clean$speaker_name[df_Debates_clean$Party=="Green, Conservative"])
table(df_Debates_clean$speaker_name[df_Debates_clean$Party=="Labour, Green"])
table(df_Debates_clean$speaker_name[df_Debates_clean$Party=="Liberal Democrat, DUP"])
table(df_Debates_clean$speaker_name[df_Debates_clean$Party=="Minister"])
table(df_Debates_clean$speaker_name[df_Debates_clean$Party=="Secretary"])


# Hand coded data frame based on the names gathered from the table before
new_party = read_excel("C:/Users/leonw/OneDrive - KU Leuven/2nd Semester/Collecting Big Data for Social Science/Final Assignment/New party.xlsx")

# Based on the previous data frame combine the names with the assigned party.
# There are no names liste twice.
df_Debates_clean <- df_Debates_clean %>%
...
df_Debates_clean$speech_content <- sub("^I beg to move","",df_Debates_clean$speech_content)
