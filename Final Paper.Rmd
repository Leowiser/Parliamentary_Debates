...
```
As we see POS tags... --> need to lemmatize and POS tag our speeches.
...
While stop word removal is a common pre-processing step in text analysis, they in fact help the model better understand the context of a word so we do not remove them and apply the pre-trained model on our raw texts (speeches).
...
# Lemmatize and POS tag speeches - BEWARE: code runs for 2.5 hours
...
```
Validate lemmatization and POS tagging by manually checking 100 speeches. Also compare it with spacy
```{r, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
...
The POS tags from UDPipe are different than in the sentiment lexicon --> need to map them so that later we can match on lemma-pos combinations.
```{r, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
unique(pos$upos)
unique(sentiment_lexicon$pos1)
```
[Insert mapping table of POSs here]
```{r, eval=TRUE, echo=TRUE, message=FALSE, warning=FALSE, results='hide'}
# Mapping POS tags
sentiment_lexicon %>%
